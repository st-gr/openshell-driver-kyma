...
sensitive namespaces (`sail-proxy`, `openwebui`, etc.).
...
- **Isolation.** Driver writes/reads stay namespace-scoped. Tier-3 tests
  ...
  protect `sail-proxy`, `openwebui`, etc. without committing those names.
...
- **User's prior Kyma work** at `C:/eclipse-workspace/sap-llm-gateway/sail-proxy-private/kyma/`: deployment patterns (PeerAuthentication, DestinationRule, sidecar injection toggles) — read for context only; do not import any cluster-specific values.
